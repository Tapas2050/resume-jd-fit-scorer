...
* **Root Cause**:
  ...
  2. *Upstream Queue Latency*: The 550B MoE primary model on shared free infrastructure occasionally experienced upstream processing delays that exceeded a 15-second client timeout during queue bursts.
* **Remediation**:
  ...
  2. *Official Project Timeout Baseline*: Based on observed Phase 10 telemetry (where extraction latency averaged 17.79s, scoring latency averaged 21.23s, and peak latency reached 34.13s), `LLM_TIMEOUT_SECONDS=45` was established as the official project baseline across the Master specification (§2.5), `.env.example`, and application defaults. The 45-second value is not a measured latency itself, but rather the configured timeout chosen to provide safe headroom for observed free-tier MoE generation while maintaining a firm ceiling against hung connections.
* **Later Verification**: In the Phase 10 calibration experiment, with the updated fallback model and the 45-second timeout configuration, all 6 live OpenRouter calls across all three candidate resumes completed successfully with zero 404 errors, zero timeouts, and 100% Pydantic schema validation.
...
* **Documented Technical Limitations**:
  ...
  3. *Upstream Free-Tier Queueing*: Free OpenRouter endpoints are subject to provider load fluctuations; when queue latency exceeds the configured 45-second timeout, the primary-to-fallback sequence is triggered as designed.
